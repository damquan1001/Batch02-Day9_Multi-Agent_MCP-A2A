...
def search_privacy_law(query: str) -> str:
    """Search privacy and data protection knowledge base for applicable rules.

    Args:
        query: Natural language query about privacy, GDPR, or data breaches.
    """
    knowledge = [
        (
            ["data", "privacy", "gdpr", "consent", "breach", "dữ liệu"],
            "GDPR can require a lawful basis for processing, notice to individuals, "
            "data minimisation, and breach notification to regulators within 72 hours "
            "when a personal-data breach creates risk to individuals.",
        ),
        (
            ["ccpa", "cpra", "consumer", "customer"],
            "CCPA/CPRA may require clear privacy notices, consumer access/deletion rights, "
            "opt-out rights for sale or sharing, and statutory damages for certain data breaches.",
        ),
        (
            ["security", "leak", "rò rỉ", "incident"],
            "Data leaks can trigger regulator investigations, customer notification duties, "
            "class action exposure, contractual indemnities, and remediation obligations.",
        ),
    ]
    query_lower = query.lower()
    results = []
...
@tool
def estimate_financial_exposure(revenue: float, severity: str) -> str:
    """Estimate financial exposure from legal and regulatory incidents.

    Args:
        revenue: Annual revenue in USD.
        severity: Severity level (low, medium, high)
    """
    multipliers = {"low": 0.01, "medium": 0.05, "high": 0.12}
    multiplier = multipliers.get(severity.lower(), 0.05)
    exposure = revenue * multiplier
    return (
        f"Estimated financial exposure: ${exposure:,.2f} "
        f"({multiplier:.0%} of ${revenue:,.2f} annual revenue). "
        "Include remediation, legal fees, customer compensation, compliance upgrades, "
        "insurance deductibles, and business interruption."
    )
...
async def check_routing(state: LegalState) -> dict:
    ...
    messages = [
        SystemMessage(
            content=(
                ...
                '"needs_privacy": <true|false>, "needs_financial": <true|false>}\n\n'
                ...
                'needs_privacy = true → question involves data, privacy, GDPR, consent, data leaks\n'
                'needs_financial = true → question involves damages, losses, revenue, cost, financial exposure'
            )
        ),
        HumanMessage(content=state["question"]),
    ]
    ...
    print(
        ...
        f"needs_privacy={needs_privacy}, needs_financial={needs_financial}"
    )
    return {
        "needs_tax": needs_tax,
...
def route_to_specialists(state: LegalState) -> list[Send]:
    ...
    if state.get("needs_financial"):
        sends.append(Send("call_financial_specialist", state))
    if not sends:
...
async def call_privacy_specialist(state: LegalState) -> dict:
    ...
    privacy_prompt = (
        "You are a senior privacy attorney with expertise in GDPR, CCPA/CPRA, "
        "data breach notification, consent, data minimisation, and privacy rights. "
...
async def call_financial_specialist(state: LegalState) -> dict:
    """Financial specialist sub-agent for damages and business exposure."""
    from langgraph.prebuilt import create_react_agent

    print("\n  [Node: call_financial_specialist] Financial specialist agent starting...")

    financial_prompt = (
        "You are a financial damages analyst for legal incidents. Estimate likely exposure "
        "across direct damages, penalties, remediation, legal fees, customer compensation, "
        "business interruption, insurance, and compliance upgrades. Use estimate_financial_exposure "
        "when revenue or severity assumptions are useful. Keep your response under 200 words."
    )

    llm = get_llm()
    agent = create_react_agent(model=llm, tools=[estimate_financial_exposure], prompt=financial_prompt)
    result = await agent.ainvoke({"messages": [{"role": "user", "content": state["question"]}]})

    final_msg = result["messages"][-1].content
    print(f"  [Node: call_financial_specialist] Done ({len(final_msg)} chars)")
    return {"financial_result": final_msg}
...
async def aggregate(state: LegalState) -> dict:
    ...
    if state.get("financial_result"):
        sections.append(f"## Financial Exposure Analysis\n{state['financial_result']}")
...
def create_graph():
    ...
    graph.add_node("call_financial_specialist", call_financial_specialist)
    ...
    graph.add_edge("call_financial_specialist", "aggregate")
...
QUESTION = (
    "If a company breaks a contract, leaks customer data without consent, and avoids taxes, "
    "what are the legal, regulatory, and financial consequences?"
)
...
async def main():
    ...
    print("  3. Tax + Compliance + Privacy + Financial specialists run IN PARALLEL (LangGraph Send API)")
    print("  4. Aggregator combines all analyses into a final answer")
    print()
    print("[Graph topology]")
    print("  analyze_law -> check_routing -> [call_tax + call_compliance + call_privacy + call_financial] -> aggregate -> END")
    print()
